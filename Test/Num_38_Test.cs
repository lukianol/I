...
namespace Test
{
    ...
    public class Num_38_Test
    {
        ...
        {
            var subArrayWithLargestSum = Main.GetSubArrayWithLargestSum(input, LinearSubArrayWithLargestSumStrategy);
            CollectionAssert.AreEqual(output, subArrayWithLargestSum);
        }

        [Test]
        [TestCaseSource("ExamplesOfArrayAndSubArrays")]
        [Ignore("Calibrating strategy was used to check the linear startegy. This strategy itself is greater than required O(n)")]
        public void CalibratingSubArrayWithLargestSumStrategy_Finds_Correct_SubArray(int[] input, int[] output)
        {
            var subArrayWithLargestSum = Main.GetSubArrayWithLargestSum(input, CalibratingSubArrayWithLargestSumStrategy);
            CollectionAssert.AreEqual(output, subArrayWithLargestSum);
        }

        [Test]
        [Repeat(100)]
        [Ignore("This test is used to compare Calibrating vs. Linear strategy. It has a random nature and may fail because of trailing zeros and zero expressions")]
        public void Calibrating_vs_Linear()
        {
            var random = new Randomizer();
            var array = random.GetInts(-100, 100, random.Next(2, 10));
            Trace.TraceInformation("Array: {0}", array.ToArrayString());
            var expected = Main.GetSubArrayWithLargestSum(array, CalibratingSubArrayWithLargestSumStrategy);
            Trace.TraceInformation("Expected: {0}", expected.ToArrayString());
            var actual = Main.GetSubArrayWithLargestSum(array, LinearSubArrayWithLargestSumStrategy);
            Trace.TraceInformation("Actual: {0}", actual.ToArrayString());
            CollectionAssert.AreEqual(expected, actual);
        }

        private static readonly CalibratingSubArrayWithLargestSumStrategy CalibratingSubArrayWithLargestSumStrategy = new CalibratingSubArrayWithLargestSumStrategy();
        private static readonly LinearSubArrayWithLargestSumStrategy LinearSubArrayWithLargestSumStrategy = new LinearSubArrayWithLargestSumStrategy();
    }
}
